...
func (c *CloudLibraryThreatCommand) Run(args []string) int {
	...
	if len(remainingArgs) < 1 {
		...
		fmt.Fprintf(os.Stderr, "Usage: threatcl cloud library threat [options] <id>\n")
		return 1
	}
	...
	// Fetch threat
	threat, err := c.fetchThreatLibraryItem(token, orgId, threatId, httpClient, apiURL)
	if err != nil {
...
func (c *CloudLibraryThreatCommand) fetchThreatLibraryItem(token, orgId, threatId string, httpClient HTTPClient, apiURL string) (*threatLibraryItem, error) {
