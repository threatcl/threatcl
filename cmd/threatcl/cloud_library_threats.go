...
func (c *CloudLibraryThreatsCommand) Run(args []string) int {
	...
	// Fetch threats
	threats, err := c.fetchThreatLibraryItems(token, orgId, filter, httpClient, apiURL)
	if err != nil {
...
func (c *CloudLibraryThreatsCommand) fetchThreatLibraryItems(token, orgId string, filter map[string]interface{}, httpClient HTTPClient, apiURL string) ([]threatLibraryItem, error) {
